...
/* Data types */
typedef enum { flagcxInt8       = 0, flagcxChar       = 0,
               flagcxUint8      = 1,
               flagcxInt32      = 2, flagcxInt        = 2,
               flagcxUint32     = 3,
               flagcxInt64      = 4,
               flagcxUint64     = 5,
               flagcxFloat16    = 6, flagcxHalf       = 6,
               flagcxFloat32    = 7, flagcxFloat      = 7,
               flagcxFloat64    = 8, flagcxDouble     = 8,
               flagcxBfloat16   = 9,
               flagcxNumTypes   = 10
} flagcxDataType_t;
...
typedef enum { flagcxMemcpyHostToDevice = 0,
               flagcxMemcpyDeviceToHost = 1,
               flagcxMemcpyDeviceToDevice = 2
} flagcxMemcpyType_t;

typedef enum { flagcxMemHost = 0,
               flagcxMemDevice = 1,
               flagcxMemManaged = 2
} flagcxMemType_t;
...
struct flagcxDeviceHandle {
   ...
   flagcxResult_t (*deviceMemset)(void *ptr, int value, size_t size, flagcxMemType_t type);
...
struct flagcxHandlerGroup {
    int* status; // 0: allocated, 1: initialized
 ...
 * Each rank is associated to a device, which has to be set before calling flagcxCommInitRank.
 * flagcxCommInitRank implicitly syncronizes with other ranks, so it must be
 * called by different threads/processes or use flagcxGroupStart/flagcxGroupEnd. */
flagcxResult_t flagcxCommInitRank(flagcxComm_t *comm, int nranks, flagcxUniqueId_t commId, int rank);

/* Finalize a communicator. flagcxCommFinalize flushes all issued communications,
 * and marks communicator state as flagcxInProgress. The state will change to flagcxSuccess
 * when the communicator is globally quiescent and related resources are freed; then,
 * calling flagcxCommDestroy can locally free the rest of the resources (e.g. communicator
 ...
 */
flagcxResult_t flagcxReduce(const void* sendbuff, void* recvbuff, size_t count, flagcxDataType_t datatype,
    flagcxRedOp_t op, int root, flagcxComm_t comm, flagcxStream_t stream);
...
flagcxResult_t flagcxGather(const void* sendbuff, void* recvbuff, size_t count,
    flagcxDataType_t datatype, int root, flagcxComm_t comm, flagcxStream_t stream);
...
flagcxResult_t flagcxAllReduce(const void* sendbuff, void* recvbuff, size_t count,
    flagcxDataType_t datatype, flagcxRedOp_t op, flagcxComm_t comm, flagcxStream_t stream);
...
flagcxResult_t flagcxReduceScatter(const void* sendbuff, void* recvbuff,
    size_t recvcount, flagcxDataType_t datatype, flagcxRedOp_t op, flagcxComm_t comm,
 ...
 *
 * Rank peer needs to call flagcxRecv with the same datatype and the same count from this
 * rank.
 *
 * This operation is blocking for the GPU. If multiple flagcxSend and flagcxRecv operations
 * need to progress concurrently to complete, they must be fused within a flagcxGroupStart/
 * flagcxGroupEnd section.
 */
 ...
 *
 * Rank peer needs to call flagcxSend with the same datatype and the same count to this
 * rank.
 *
 * This operation is blocking for the GPU. If multiple flagcxSend and flagcxRecv operations
 * need to progress concurrently to complete, they must be fused within a flagcxGroupStart/
 ...
 *
 * Both collective communication and flagcxCommInitRank can be used in conjunction
 * of flagcxGroupStart/flagcxGroupEnd, but not together.
 *
 ...
 * Group Start
 *
 * Start a group call. All calls to FLAGCX until flagcxGroupEnd will be fused into
 ...
 * Group End
 *
 * End a group call. Start a fused FLAGCX operation consisting of all calls since
