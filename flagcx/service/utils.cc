...
__thread struct flagcxThreadSignal flagcxThreadSignalLocalInstance = flagcxThreadSignalStaticInitializer();

void* flagcxMemoryStack::allocateSpilled(struct flagcxMemoryStack* me, size_t size, size_t align) {
  ...
  { // If the next hunk we're going to allocate wouldn't be big enough but the
    ...
    mallocSize = nextSize;
    INFO(FLAGCX_ALLOC, "%s:%d memory stack hunk malloc(%llu)", __FILE__, __LINE__, (unsigned long long)mallocSize);
...
unhunked:
  { // We need to allocate the object out-of-band and put an Unhunk proxy in-band
    ...
    INFO(FLAGCX_ALLOC, "%s:%d memory stack non-hunk malloc(%llu)", __FILE__, __LINE__, (unsigned long long)mallocSize);
