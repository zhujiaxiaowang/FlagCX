...
struct flagcxTransportComm {
  flagcxResult_t (*setup)(struct flagcxHeteroComm* comm, struct flagcxTopoGraph* graph, struct flagcxPeerInfo*, struct flagcxPeerInfo*, struct flagcxConnect*, struct flagcxConnector*, int channelId, int connIndex);
  flagcxResult_t (*connect)(struct flagcxHeteroComm* comm, struct flagcxConnect*, int nranks, int rank, struct flagcxConnector*);
  flagcxResult_t (*free)(struct flagcxConnector*);
  flagcxResult_t (*proxySharedInit)(struct flagcxProxyConnection* connection, struct flagcxProxyState* proxyState, int nChannels);
  flagcxResult_t (*proxySetup)(struct flagcxProxyConnection* connection, struct flagcxProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done);
  flagcxResult_t (*proxyConnect)(struct flagcxProxyConnection* connection, struct flagcxProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done);
  flagcxResult_t (*proxyFree)(struct flagcxProxyConnection* connection, struct flagcxProxyState* proxyState);
  flagcxResult_t (*proxyProgress)(struct flagcxProxyState* proxyState, struct flagcxProxyArgs*);
  flagcxResult_t (*proxyRegister)(struct flagcxProxyConnection* connection, struct flagcxProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done);
  flagcxResult_t (*proxyDeregister)(struct flagcxProxyConnection* connection, struct flagcxProxyState* proxyState, void* reqBuff, int reqSize, int* done);
};

struct flagcxTransport {
  const char name[8];
  flagcxResult_t (*canConnect)(int*, struct flagcxTopoSystem* topo, struct flagcxTopoGraph* graph, struct flagcxPeerInfo*, struct flagcxPeerInfo*);
  struct flagcxTransportComm send;
  struct flagcxTransportComm recv;
};

flagcxResult_t flagcxTransportP2pConnect(struct flagcxHeteroComm* comm, int channelId, int nrecv, int* peerRecv, int nsend, int* peerSend, int connIndex);
...
flagcxResult_t flagcxNvlsSetup(struct flagcxHeteroComm* comm, struct flagcxHeteroComm* parent);
flagcxResult_t flagcxNvlsGraphRegisterBuffer(struct flagcxHeteroComm *comm, struct flagcxKernelPlan *plan, const void *sendbuff, void *recvbuff, size_t sendbuffSize, size_t recvbuffSize, bool *outRegBufUsed, void **outRegBufSend, void **outRegBufRecv);
flagcxResult_t flagcxNvlsLocalRegisterBuffer(struct flagcxHeteroComm *comm, const void *sendbuff, void *recvbuff, size_t sendbuffSize, size_t recvbuffSize, bool *outRegBufUsed, void **outRegBufSend, void **outRegBufRecv);
