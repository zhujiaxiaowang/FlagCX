...
flagcxResult_t flagcxTransportP2pSetup(struct flagcxHeteroComm* comm, struct flagcxTopoGraph* graph, int connIndex, int* highestTransportType/*=NULL*/) {
  ...
  for(int peer=0;peer<comm->nRanks;peer++){
    for(int c=0;c<MAXCHANNELS;c++){
      if(comm->connectRecv[peer] & (1UL<<c)){
        struct flagcxConnector* conn = comm->channels[c].peers[peer]->recv + connIndex;
        ...
        flagcxNetIb.listen(resources->netDev,(void *) handle, &resources->netListenComm);
        bootstrapSend(comm->bootstrap, peer, 1001+c, handle, sizeof(flagcxIbHandle));
        deviceAdaptor->streamCreate(&resources->cpStream);
        resources->buffSizes[0] = REGMRBUFFERSIZE;
        deviceAdaptor->gdrMemAlloc((void **)&resources->buffers[0], resources->buffSizes[0], NULL);
        FLAGCXCHECK(flagcxProxyCallAsync(comm, &conn->proxyConn, flagcxProxyMsgConnect, handle, sizeof(flagcxIbHandle), 0, conn));

        free(handle);
      }

      if(comm->connectSend[peer] & (1UL<<c)){
        struct flagcxConnector* conn = comm->channels[c].peers[peer]->send + connIndex;
        FLAGCXCHECK(flagcxCalloc(&conn->proxyConn.connection, 1));
        ...
        conn->proxyConn.connection->transportResources = (void *)resources;
        resources->netDev = comm->netDev;
        bootstrapRecv(comm->bootstrap, peer, 1001+c, handle, sizeof(flagcxIbHandle));
  ...
  for(int peer=0;peer<comm->nRanks;peer++){
    for(int c=0;c<MAXCHANNELS;c++){
      if(comm->connectRecv[peer] & (1UL<<c)){
        struct flagcxConnector* conn = comm->channels[c].peers[peer]->recv + connIndex;
        while(flagcxPollProxyResponse(comm, NULL, NULL, conn) == flagcxInProgress);
