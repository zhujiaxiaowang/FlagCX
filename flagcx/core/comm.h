...
struct flagcxCommCallback {
  ...
  flagcxResult_t(*fn)(struct flagcxHeteroComm* comm, struct flagcxCommCallback* cb);
};
...
struct flagcxHeteroComm {
  ...
  float latencies[FLAGCX_NUM_FUNCTIONS][FLAGCX_NUM_ALGORITHMS][FLAGCX_NUM_PROTOCOLS];
  float bandwidths[FLAGCX_NUM_FUNCTIONS][FLAGCX_NUM_ALGORITHMS][FLAGCX_NUM_PROTOCOLS];
  ...
  // Intra-process sync
  struct flagcxHeteroComm* intraComm0; // leader of intra-process comms (self possible)
  struct flagcxHeteroComm* intraNext; // next of intra-process comms, intraComm0 is head
  int intraRank;
  int intraRanks;
  ...
  // Next comm in this thread's active flagcxGroup[Start|End](). Holds "0x1" when
  ...
  struct flagcxIntruQueueMpsc<struct flagcxCommCallback, &flagcxCommCallback::next> callbackQueue;
...
inline flagcxResult_t flagcxCommPollCallbacks(struct flagcxHeteroComm* comm, bool waitSome) {
  flagcxResult_t result = flagcxSuccess;
  struct flagcxCommCallback* cb = flagcxIntruQueueMpscDequeueAll(&comm->callbackQueue, waitSome);
  while (cb != nullptr) {
    struct flagcxCommCallback* next = cb->next;
    flagcxResult_t res1 = cb->fn(comm, cb); // may reclaim memory of cb
    if (res1 != flagcxSuccess) result = res1;
    cb = next;
  }
  FLAGCXCHECK(result);
...
inline void flagcxCommIntraBarrierIn(struct flagcxHeteroComm* comm, uint32_t x) {
...
static inline flagcxRedOp_t flagcxUserRedOpMangle(flagcxHeteroComm *comm, flagcxRedOp_t op) {
...
flagcxResult_t flagcxCommSetAsyncError(flagcxHeteroComm_t comm, flagcxResult_t nextState);
