...
// static flagcxResult_t getPath(struct flagcxTopoServer* system, struct
// flagcxTopoNode* node, int t, int64_t id, struct flagcxTopoPath** path) {
...
// struct flagcxTopoServer* system) {
//   if (baseNode->paths[baseNode->type] == NULL) {
//     FLAGCXCHECK(flagcxCalloc(baseNode->paths+baseNode->type,
//     system->nodes[baseNode->type].count));
//   }
...
//   FLAGCXCHECK(getPath(system, baseNode, baseNode->type, baseNode->id,
//   &basePath)); basePath->count = 0; basePath->bw = LOC_BW; basePath->type =
//   PATH_LOC;
...
//       FLAGCXCHECK(getPath(system, node, baseNode->type, baseNode->id,
//       &path)); for (int l=0; l<node->nlinks; l++) {
...
//           system->nodes[baseNode->type].count)); for (int i=0;
//           i<system->nodes[baseNode->type].count; i++)
//           remNode->paths[baseNode->type][i].type = PATH_DIS;
//         }
//         struct flagcxTopoPath* remPath;
//         FLAGCXCHECK(getPath(system, remNode, baseNode->type, baseNode->id,
//         &remPath)); float bw = std::min(path->bw, link->bw);

//         // allow routing through a GPU only as 1 hop
//         if (node != baseNode && node->type == GPU &&
//             (flagcxParamNvbDisable() || link->type != LINK_NVL ||
//             remNode->type != GPU || path->count > 1)) continue;

//         if ((remPath->bw == 0 || remPath->count > path->count) && remPath->bw
//         < bw) {
//           // Find reverse link
//           for (int l=0; l<remNode->nlinks; l++) {
//             if (remNode->links[l].remNode == node && remNode->links[l].type
//             == link->type) {
...
//             WARN("Failed to find reverse path from remNode %d/%lx nlinks %d
//             to node %d/%lx",
...
//           for (int i=0; i<path->count; i++) remPath->list[i+1] =
//           path->list[i]; remPath->count = path->count + 1; remPath->bw = bw;

//           // Start with path type = link type. PATH and LINK types are
//           supposed to match.
...
//           if (link->type == LINK_PCI && (node->type == CPU ||
//           link->remNode->type == CPU)) type = PATH_PHB;
//           // Set 1 hop NVLink as NVB
//           if (node->type == GPU && path->type == PATH_NVL && type == PATH_NVL
//           && remPath->count > 1) type = PATH_NVB;

//           remPath->type = std::max(path->type, type);

//           // Add to the list for the next iteration if not already in the
//           list int i; for (i=0; i<nextNodeList.count; i++) if
//           (nextNodeList.list[i] == remNode) break; if (i ==
//           nextNodeList.count) nextNodeList.list[nextNodeList.count++] =
//           remNode;
//         }
//       }
//     }
...
// static void printNodePaths(struct flagcxTopoServer* system, struct
// flagcxTopoNode* node) {
//   const int linesize = 1024;
//   char line[linesize];
// #ifdef ENABLE_TRACE
//   INFO(FLAGCX_GRAPH, "Paths from %s/%lX :", topoNodeTypeStr[node->type],
//   node->id);
// #else
//   snprintf(line, linesize, "%s/%lX :", topoNodeTypeStr[node->type],
//   node->id); int offset = strlen(line);
// #endif
...
//         FLAGCX_TOPO_ID_SYSTEM_ID(remNode->id),
//         FLAGCX_TOPO_ID_LOCAL_ID(remNode->id)); offset = strlen(line);
//       }
...
//       topoPathTypeStr[node->paths[t][n].type]); offset = strlen(line);
// #endif
//     }
//   }
// #ifndef ENABLE_TRACE
...
// flagcxResult_t flagcxTopoPrintPaths(struct flagcxTopoServer* system) {
...
// static void flagcxTopoRemovePathType(struct flagcxTopoServer* system, int
// nodeType) {
//   for (int t=0; t<FLAGCX_TOPO_NODE_TYPES; t++) {
...
// flagcxResult_t flagcxTopoComputePaths(struct flagcxTopoServer* system, struct
// flagcxHeteroComm* comm) {
//   // Precompute paths between GPUs/NICs.

//   // Remove everything in case we're re-computing
//   for (int t=0; t<FLAGCX_TOPO_NODE_TYPES; t++)
//   flagcxTopoRemovePathType(system, t);
...
//   // TODO: Update path for GPUs when we don't want to / can't use GPU Direct
//   P2P
